...
object oclGroupedReducePrivateSeqInitAggregate extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    override val name: String = "oclGroupedReducePrivateSeqInitAggregate"
    ...
    override def typeScheme: ExprType = expl { (a: AddressSpace) => expl { (n: Nat) => expl { (m: Nat) => impl { (t: DataType) => impl { (u: DataType) => (t ->: t ->: t) ->: u ->: (IndexType(m * n) ->: t) ->: u } } } } }
  }
  override def toString: String = "oclGroupedReducePrivateSeqInitAggregate"
