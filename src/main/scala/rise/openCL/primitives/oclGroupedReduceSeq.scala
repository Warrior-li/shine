...
object oclGroupedReduceSeq extends Builder {
  private final case class Primitive()(override val t: ExprType = TypePlaceholder) extends rise.core.Primitive {
    ...
    override def typeScheme: ExprType = expl { (a: AddressSpace) => expl { (n: Nat) => expl { (m: Nat) => impl { (t: DataType) => (t ->: t ->: t) ->: ArrayType(m * n, t) ->: ArrayType(m, t) } } } }
  }
  override def toString: String = "oclGroupedReduceSeq"
