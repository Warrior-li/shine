...
final case class GroupedReduceSeqNested(
  val a: AddressSpace,
  val n: Nat,
  val m: Nat,
  val dt: DataType,
  ...
  override def visitAndRebuild(v: VisitAndRebuild.Visitor): GroupedReduceSeqNested =
    new GroupedReduceSeqNested(v.addressSpace(a), v.nat(n), v.nat(m), v.data(dt), VisitAndRebuild(f, v), VisitAndRebuild(input, v))
}
