...
//  def showPhraseType : PhraseType => String = _.toString
  ...
  def showPrim : Primitive[_] => String = {
    ...
    case Drop(n, _, _, _, e) => s"drop ${showNat(n)} ${showTypedPhrase(e)}"
    case DropAcc(n, _, _, e) => s"drop ${showNat(n)} ${showTypedPhrase(e)}"
    case Take(n, _, _, _, e) => s"take ${showNat(n)} ${showTypedPhrase(e)}"
    ...
    case StaticIterate(_, _, f, init) => s"staticIterate ${showPhrase(init)}\n${tab(showPhrase(f))}"
    ...
    case Materialize(_, input) => s"materialize ${showPhrase(input)}"
    case LocalOwner(_, input) => s"localOwner ${showPhrase(input)}"
    case ProjectWrite(_, _, _, _, _, input) => s"projectWrite ${showPhrase(input)}"
